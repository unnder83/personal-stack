...
| M6 运维一期 | 监控大盘、每日备份 + 恢复演练、安全加固 | Grafana 有数据；从备份成功恢复一次；加固清单逐项验收：容器 `cap_drop: ALL` 与可只读的根文件系统、CSP/HSTS 响应头、基础镜像按 digest 固定、fail2ban、宿主重启后服务自动恢复（rsync 等工具装齐）。**SSH 密钥化与禁 root 登录按用户决定暂缓**（继续密码登录 + fail2ban 兜底），其余项执行 |
